...
try {
await axios.get("https://api.github.com/repos/elijahducote/djev/contents/automation.json",{headers:{"Accept":"application/vnd.github+json","Authorization":`Bearer ${process.env.TOKEN}`,"X-GitHub-Api-Version":"2022-11-28"}})
 .then(response => {
...
await axios.put("https://api.github.com/repos/elijahducote/djev/contents/automation.json",{"message":"update file","sha":sha,"content":Buffer.from(fyl).toString("base64")},{headers:{"Accept":"application/vnd.github+json","Authorization":`Bearer ${process.env.TOKEN}`,"X-GitHub-Api-Version":"2022-11-28"}}).then(response => {
    if (response.status === 200) {
      msgcode = response.commit.sha;
    }
    else throw new Error("Uh, oh! " + response.status);
 });
 return {
